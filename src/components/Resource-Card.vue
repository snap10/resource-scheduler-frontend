    ...
    <div class="card-content">
      <div class="media">
        <div class="media-content">
          ...
          <p class="heading ">von {{owner}}</p>
          <p class="heading " v-if="resource.organisationId">Verantwortlich: {{responsibleText}}</p>
          <p class="heading">&nbsp;</p>
          <div class="content" v-if="eventsLoaded">
            <div v-if="reservations&&reservations.length>0">
              <p class="heading ">Neueste Reservierungen:</p>
              <ul>
                <li :key="computedResourceId+'-'+res.id" v-for="res in reservations"><time datetime="res.startDatetime">{{res.name}}
                    {{res.startDatetime | moment("from") }}<span v-if="isCurrently(res)" class="has-text-danger">&nbsp;(laufend)</span></time></li>
              </ul>
            </div>
...
<script>
  export default {
    ...
    created() {
      ...
      if (this.resource.organisationId && !this.$store.getters.organisation(this.resource.organisationId)) this.$store.dispatch(
        'loadOrganisation', this.resource.organisationId)
    ...
    computed: {
      ...
      responsible() {
        var value = (this.$store.getters.user(this.resource.creatorId)) ? this.$store.getters.user(this.resource.creatorId)
          .name : null
      ...
      owner() {
        ...
        value = (value && this.$store.getters.organisation(value)) ? this.$store.getters.organisation(value).name :
          value
